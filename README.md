...
## v0.8.0 更新
- 新增 **本地词汇记忆**：用户接受的词条会写入本地库，后续 AI 清理 raw 转写前按同音/近音预筛后注入，不会全量塞进提示词。
- 新增 **raw-only 词汇审查**：设置页可手动运行词汇审查；模型只看原始 ASR 转写片段，基于整句上下文提出候选，不再把 AI 清理结果当作正确答案。
- 精简旧审查流：移除脏话/内容质量/边界越界/提示词失败等广义审查卡片，只保留可人工接受的 `old_form -> new_form` 词汇候选。
- 移除 AI 输出实时拒绝门禁：AI 清理结果不再因为本地 validator 的压缩、翻译、越界等规则被自动回退；低信息输入跳过逻辑仍保留。
...
- 审查模型会先根据整句 raw 上下文判断候选词是否合理；候选必须由用户接受后才会进入本地词汇记忆。
- 后续 AI 清理前，系统用 `LexiconMatcher` 对当前 raw 文本做同音/近音预筛，只注入相关词条。
- 自动词汇审查默认关闭；可在设置页手动运行，也可开启空闲调度。旧版审查数据会保留在本地数据库中，但不再作为新审查流的一部分展示或接受。
