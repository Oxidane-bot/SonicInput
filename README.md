...
- 本地学习：本地规则审查可从历史记录发现术语候选和质量问题，用户接受后才进入本地词汇记忆
...
- 新增 **本地质量审查页面**：设置窗口中可查看本地规则审查发现的待处理建议，按边界越界、内容失真、诊断样本、词汇记忆和提示词问题分组处理。
...
- 本地质量审查默认关闭自动调度；可在设置页手动运行本地审查，检查最近历史中的质量建议。
- 本地规则审查不会自动改写历史记录；接受词汇候选只会加入本地词汇记忆，后续 AI 清理会把它作为保守参考。
